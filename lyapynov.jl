...
# ╔═╡ 24ed2c7c-6189-11ef-2b23-8d4a52030ca8
using Images, StaticArrays, PlutoUI
...
# ╔═╡ 60f6bbee-8cad-4d8d-abf9-51369fde10e8
pointr(point, char) = char == 'a' ? point.x : point.y

# ╔═╡ 0ad24d69-533d-4da9-a268-20b604b29e1e
function next(point, prev, char)
	r = pointr(point, char)
	r * prev * (1 - prev)
end
...
function lyapynov(point, string, N)
	...
	acc = xn
	for n in 2:N
		char = string[mod1(n, length(string))]
		r = pointr(point, char)
		acc += log(abs(r * (1 - 2xn)))
		xn = next(point, xn, char)
	end
	acc/N
end
...
function render(xrange, yrange, string, N)
	ret = Matrix{RGB{N0f8}}(undef, length(xrange), length(yrange))
...
manifest_format = "2.0"
project_hash = "32503f977085462f74fd4a3994eeb908f3cd12fc"
